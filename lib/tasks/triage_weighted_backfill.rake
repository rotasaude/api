...
#
# Conexão: roda cidade a cidade (CityConnection.with), nas cidades ativas do
# catálogo ou só em CITY_SLUG.
#
...
#   CITY_SLUG=curitiba bin/rails triage:audit_weighted_scoring  # só uma cidade
namespace :triage do
  ...
  task audit_weighted_scoring: :environment do
    ...
    cities = ENV["CITY_SLUG"].present? ? City.where(slug: ENV["CITY_SLUG"]) : City.active
    cities.find_each do |city|
      puts "[audit] cidade: #{city.slug}"
