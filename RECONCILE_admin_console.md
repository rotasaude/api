...
- **Auth real** via cookie de sessão HttpOnly (ADR-0011). User + Session
...
|---|---|---|---|---|
| §4.1 | `inbound_messages.status` (ack/erro) | só `created_at,from,kind,message_id,raw` | `ack[]` aproximado por `outbound_messages.status` | projeção `ingestion_metrics` (ADR 0010) |
| §4.1 | `inbound_messages.processed`, `raw_purged_at` (backlog LGPD) | não existem | `purge.pending` = count(idade > 24h); `oldestH` = (now - min(created_at))/1h | projeção `purge_backlog` ou colunas (ADR 0014) |
...
| §4.6 | `created_by`/`published_by` por versão (quatro-olhos) | `ProtocolDefinition` não tem essas colunas | lemos de `domain_events.payload['actor']` para `protocol.created`/`protocol.published`; `fourEyes: null` quando ausentes | colunas em `protocol_definitions` (ADR 0009) |
...
| §2.4 | RBAC, sessão real | ~~apenas `Author.token` (stub)~~ → **resolvido em ADR-0011** | Rails 8 native auth: `User` + `Session`, cookie HttpOnly. `Admin::Api::BaseController` inclui `Authentication` concern; `require_authentication` é before_action. | RBAC (roles) quando entrar mutação na Fase 2 |
...
| §2 | cross-tenant superadmin (`?municipality_id=all`) | sem flag em User | `cross_tenant?` retorna `false` por enquanto → `municipality_id=all` ignorado silenciosamente | coluna/flag em User (ADR-0003 quando precisar) |
...
1. ~~ADR de auth pendente — bloqueio para produção.~~ **Resolvido em
   [ADR-0011](../../docs/adr/0011.md)**: Rails 8 native auth (User + Session,
...
4. **Projeções novas = ADR de read-side (ADR 0010).** Os campos null acima
